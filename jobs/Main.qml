...
// Background singleton that subscribes to the zsa.oryx.Jobs DBus service
// via `gdbus monitor` and exposes the current job state to BarWidget and Panel.
//
...
Item {
    ...
    // Map of job_id (string key) -> job object:
    ...
    // Extract a typed value from a GVariant dict string printed by gdbus monitor.
    // Dict looks like: {'key': <uint32 42>, 'other': <'hello'>}
    function extractDictValue(dict, key) {
        ...
        if (!m)
            return null
        var raw = m[1].trim()
        // Strip GVariant type prefixes
        raw = raw.replace(/^u?int\d+\s+/, "")   // uint32 N, int32 N, uint64 N …
        raw = raw.replace(/^double\s+/, "")       // double 1.0
        raw = raw.replace(/^byte\s+/, "")         // byte N
        // Unquote strings: 'hello' -> hello
    ...
    function parseSignalLine(line) {
        ...
        if (state === "created") {
            // All dict entries are metadata key/value pairs
            var meta = {}
            var metaRe = /'([^']+)':\s*<([^>]*)>/g
            var mm
